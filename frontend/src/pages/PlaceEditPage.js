...
const PlaceEditPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [placeRes, categoriesRes, placeCategoriesRes, citiesRes] = await Promise.all([
          axios.get(`http://localhost:5000/places/${id}`),
          axios.get('http://localhost:5000/categories'),
          axios.get(`http://localhost:5000/places/${id}/categories`),
        ...
        setSelectedCategories(placeCategoriesRes.data.map((cat) => cat.id));
        setCities(citiesRes.data);

        if (placeData.latitude && placeData.longitude) {
          setMapCenter([parseFloat(placeData.latitude), parseFloat(placeData.longitude)]);
        } else {
          const selectedCity = citiesRes.data.find(city => city.id === placeData.city);
        ...
        if (placeData.photo) {
          const isExternal = placeData.photo.startsWith('http');
          setPreviewImage(isExternal ? placeData.photo : `http://localhost:5000${placeData.photo}`);
          if (isExternal) {
            setImageUrl(placeData.photo);
            setActiveTab('url');
          }
        }
        ...
        setError(err.response?.data?.message || 'Failed to load place or categories');
        setLoading(false);
      }
    };
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'city') {
      ...
      if (selectedCity && selectedCity.latitude && selectedCity.longitude && (!place.latitude || !place.longitude)) {
        setMapCenter([parseFloat(selectedCity.latitude), parseFloat(selectedCity.longitude)]);
      }
    }
  };
...
const { t } = useTranslation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (!previewImage && place.photo) {
      ...
      navigate('/adminpanel', { state: { success: 'Changes saved successfully!' } });
    } catch (err) {
  ...
  if (!place) {
  return <Alert variant="danger">{t('placeEdit.notFound')}</Alert>;
    ...
    <div className="page-container">
      ...
      <div className="mt-4 mx-5">
        <div className="d-flex justify-content-between align-items-center mt-4 mb-4">
          <h2 style={{fontWeight: "bold"}}>{t('placeEdit.title')}</h2>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  <span className="ms-2">{t('placeEdit.saving')}</span>
                </>
              ) : (
                  t('placeEdit.save')
              )}
