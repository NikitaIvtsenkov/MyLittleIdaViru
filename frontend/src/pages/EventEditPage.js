...
//     setError(null);
//     try {
//       const [eventRes, placesRes, citiesRes] = await Promise.all([
//         axios.get(`http://localhost:5000/events/${id}`).catch(err => {
//           throw new Error(`Failed to load event: ${err.message}`);
//         }),
//         axios.get('http://localhost:5000/places').catch(err => {
//           throw new Error(`Failed to load places: ${err.message}`);
//         }),
//         axios.get('http://localhost:5000/cities').catch(err => {
//           throw new Error(`Failed to load cities: ${err.message}`);
//         })
//       ]);
...
//       if (eventData.image) {
//         const isExternal = eventData.image.startsWith('http');
//         setPreviewImage(isExternal ? eventData.image : `http://localhost:5000${eventData.image}`);
//         if (isExternal) {
...
//       setError(err.message || 'Failed to load event, places, or cities');
//       setLoading(false);
//     }
//   };
...
//       } else if (!previewImage && event.image) {
...
//   if (!event.name && !error) {
//     return <Alert variant="danger">{t('eventEdit.notFound')}</Alert>;
//   }
...
//                       >
//                         <option value="">{places.length === 0 ? t('eventEdit.noPlaces') : t('eventEdit.selectPlace')}</option>
//                         {places.map(place => (
...
//                       >
//                         <option value="">{cities.length === 0 ? t('eventEdit.noCities') : t('eventEdit.selectCity')}</option>
//                         {cities.map(city => (
...
//     setError(null);
//     try {
//       const [eventRes, citiesRes] = await Promise.all([
//         axios.get(`http://localhost:5000/events/${id}`).catch(err => {
//           throw new Error(t('eventEdit.notFound'));
//         }),
//         axios.get('http://localhost:5000/cities').catch(err => {
//           throw new Error(t('eventEdit.noCities'));
//         })
//       ]);
...
//     } catch (err) {
//       console.error('Error fetching data:', err);
//       setError(err.message || t('eventEdit.notFound'));
//       setLoading(false);
//     }
//   };
...
//       navigate('/adminpanel', {
//         state: { success: t('eventEdit.save') }
//       });
//     } catch (err) {
//       console.error('Update error:', err);
...
//   const handleReset = () => {
//     if (window.confirm(t('eventEdit.reset'))) {
//       fetchEventAndCities();
//     }
//   };
